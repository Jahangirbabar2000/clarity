...
type TicketRow = {
  ...
  createdAt: string;
  subtasks: { id: string }[];
  sprintAssignments: { sprintId: string }[];
};
...
export default function WorkspacePage() {
  ...
  const { data, isLoading } = useQuery<{ tickets: TicketRow[] }>({
    ...
    queryFn: async () => {
      const r = await fetch("/api/workspace/tickets");
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center gap-2 p-10 text-center">
              ...
              <Link href="/workspace/new"><Button><Sparkles className="h-4 w-4" /> Create first ticket</Button></Link>
            </div>
          ) : (
            <div className="divide-y">
              ...
              {tickets.map((t) => (
                <Link
                  key={t.id}
                  href={`/workspace/${t.id}`}
                  className="grid grid-cols-2 gap-2 px-4 py-3 text-sm transition-colors hover:bg-accent/40 md:grid-cols-12 md:items-center"
                >
                  ...
                  <div className="md:col-span-1">
                    <Badge variant={t.status === "PUSHED" ? "success" : t.status === "REVIEWED" ? "info" : "secondary"}>{t.status}</Badge>
                  </div>
